'use client'

import { useCallback } from 'react'
import { SCENES } from '@/lib/types'
import { ScrollTrigger } from 'gsap/ScrollTrigger'

interface SceneNavProps {
  activeScene: number
  hasLive: boolean
  onNavigate?: (index: number) => void
}

export default function SceneNav({ activeScene, hasLive, onNavigate }: SceneNavProps) {
  const handleNavClick = useCallback(
    (index: number) => {
      if (onNavigate) {
        onNavigate(index)
        return
      }

      const isMobile = window.innerWidth < 768

      if (isMobile) {
        const section = document.getElementById(SCENES[index].id)
        section?.scrollIntoView({ behavior: 'smooth', block: 'start' })
      } else {
        const triggers = ScrollTrigger.getAll()
        const trigger = triggers.find((t) => t.vars.id === 'horizontal-scroll') || triggers[0]
        if (trigger) {
          const progress = index / (SCENES.length - 1)
          const targetY = trigger.start + (trigger.end - trigger.start) * progress
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const lenis = (window as any).__lenis
          if (lenis) {
            lenis.scrollTo(targetY, { duration: 1.0 })
          } else {
            window.scrollTo({ top: targetY, behavior: 'smooth' })
          }
        }
      }
    },
    [onNavigate]
  )

  const navItems = [
    { label: 'LIVE', sceneIndex: 2 },
    { label: 'TOURNAMENTS', sceneIndex: 1 },
    { label: 'RESULTS', sceneIndex: 4 },
    { label: 'ARENA', sceneIndex: 5 },
  ]

  return (
    <nav
      className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 sm:px-6 md:px-12 h-14 sm:h-16 border-b border-border/80"
      style={{ backgroundColor: 'rgba(10, 10, 10, 0.90)', backdropFilter: 'blur(16px)' }}
      role="navigation"
      aria-label="Main navigation"
    >
      {/* Brand Logo / Home */}
      <button
        onClick={() => handleNavClick(0)}
        className="flex items-center gap-2 sm:gap-3 text-left group cursor-pointer shrink-0"
        aria-label="Triple Stars - Return to home"
      >
        <span className="font-display text-xs sm:text-sm md:text-base font-bold tracking-widest text-text-primary group-hover:text-accent transition-colors">
          TRIPLE STARS
        </span>
        <span className="hidden sm:inline-block font-mono text-[10px] text-text-secondary border border-border px-1.5 py-0.5 tracking-wider">
          ARENA
        </span>
      </button>

      {/* Nav links */}
      <div className="flex items-center gap-3 sm:gap-6 md:gap-8 overflow-x-auto no-scrollbar py-1">
        {navItems.map((item) => (
          <button
            key={item.label}
            onClick={() => handleNavClick(item.sceneIndex)}
            className={`font-mono text-[11px] sm:text-xs tracking-wider transition-all relative py-1 shrink-0 cursor-pointer ${
              activeScene === item.sceneIndex
                ? 'text-text-primary font-bold text-accent'
                : 'text-text-secondary hover:text-text-primary'
            }`}
            aria-current={activeScene === item.sceneIndex ? 'page' : undefined}
          >
            {item.label === 'LIVE' && hasLive && (
              <span className="status-dot status-live mr-1.5 inline-block -translate-y-px" />
            )}
            {item.label}
            {activeScene === item.sceneIndex && (
              <span className="absolute bottom-0 left-0 right-0 h-[2px] bg-accent" />
            )}
          </button>
        ))}

        {/* Direct Admin Control Console */}
        <a
          href="/admin"
          className="font-mono text-[10px] sm:text-[11px] font-bold tracking-wider px-2 sm:px-3 py-1 sm:py-1.5 rounded border border-accent/60 bg-accent/10 text-accent hover:bg-accent hover:text-black transition-all shrink-0"
        >
          ⚙ ADMIN
        </a>
      </div>
    </nav>
  )
}
